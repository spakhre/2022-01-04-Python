...
def main():
    ...
    # DF has 5 rows and 24 columns (indexed by title); 5 by default but here we are requesting 8
    print(movies_sheet1.head(8))
    print('\nsheet1')

    # grab the next 2 sheets as well
    movies_sheet2 = pd.read_excel(excel_file, sheet_name=1, index_col=0)
    # DF has 5 rows and 24 columns (indexed by title)
    print(movies_sheet2.head(2))
    print('\nsheet2')

    movies_sheet3 = pd.read_excel(excel_file, sheet_name=2, index_col=0)
    # DF has 5 rows and 24 columns (indexed by title)
    print(movies_sheet3.head(1))
    print('\nsheet3')
    ...
    print('\nFollowing are Sorted by gross earnings')
